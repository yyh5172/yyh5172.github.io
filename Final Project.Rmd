...
#### 4. Six States Report Comparison
...
SixStates_Comparison %>%
  ...
  ggplot(aes(x = `State/Area`, y = Count)) +
  geom_bar(stat = "identity", position = 'dodge', aes(color = `State Report`, fill = `State Report`, alpha = 0.25)) +
  labs(title = "Six States Comparison 2015 & 2016") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1), legend.position = "bottom", legend.background = element_rect(fill = "#e6e6ff", size = 0.5, linetype = "solid")) +
  facet_grid(Year ~.)

ggsave("SixStatesReportComparison.png", height = 6, width = 8)
```


#### 5. Age Group Comparison in USA
...
#### 5. Age Group Comparison in Pennsylvania
...
PA2015_AgeGroup <-
  AgeGroup2015_Clean %>%
  filter(`State/Area` == "Pennsylvania")

PA2016_AgeGroup <-
  AgeGroup2016_Clean %>%
  filter(`State/Area` == "Pennsylvania")

AgeGroup_Comparison_PA <-
  rbind(PA2015_AgeGroup, PA2016_AgeGroup) %>%
  select(`State/Area`, `Age group yrs (<15)`,`Age group yrs (15–19)`, `Age group yrs (20–24)`, `Age group yrs (25–29)`, `Age group yrs (30–34)`, `Age group yrs (35–39)`, `Age group yrs (>=40)` ) %>%
  mutate(Year = c("2015", "2016")) %>%
  pivot_longer(cols = c(`Age group yrs (<15)`, `Age group yrs (15–19)`, `Age group yrs (20–24)`, `Age group yrs (25–29)`, `Age group yrs (30–34)`, `Age group yrs (35–39)`, `Age group yrs (>=40)`), names_to ="Age Group", values_to = "Count")
kable(AgeGroup_Comparison_PA)

AgeGroup_Comparison_PA %>%
  ggplot(aes(x = `Age Group`, y = Count)) +
  geom_bar(stat = "identity", position = 'dodge', aes(color = `Age Group`, fill = `Age Group`, alpha = 0.25)) +
  labs(title = "Age Group Comparison 2015 & 2016 in Pennsylvania") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1), legend.position = "bottom", legend.background = element_rect(fill = "#e6e6ff", size = 0.5, linetype = "solid")) +
  facet_wrap( ~ Year )

ggsave("AgeGroupComparison_PA.png", height = 8, width = 10)
```
